...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    val lawnSize = lines.headOption.flatMap(line => line.split(" ").toList match {
      //Tuple
    ...
    // Exécution des instructions pour chaque tondeuse
    val finalPositions = executeInstructions(lawn, mowers)

    def writesPosition(position: Position) = JsObject(Seq(
      "x" -> Json.toJson(position.x),
      "y" -> Json.toJson(position.y),
    ))

    def writesOrientation(orientation: Orientation) = JsObject(Seq(
      "orientation" -> Json.toJson(orientation.toString),
    ))
    def writesInstructions(instructions: List[Instruction]) = JsObject(Seq(
      //List(RotateLeft, MoveForward, RotateLeft, MoveForward, RotateLeft, MoveForward, RotateLeft, MoveForward, MoveForward)
      //i want to convert to RotateLeft, MoveForward, RotateLeft, MoveForward, RotateLeft, MoveForward, RotateLeft, MoveForward, MoveForward
      "instructions" -> Json.toJson(instructions.map(_.toString)),
    ))
    val json: JsValue = Json.toJson(finalPositions.map {
      case (position, orientation,instructions) => JsObject(Seq(
        "debut" -> JsObject(Seq(
          "point" -> writesPosition(position),
          "direction" -> writesOrientation(orientation)
        )),
        "instructions" -> writesInstructions(instructions),
        "fin" -> JsObject(Seq(
          "point" -> writesPosition(position),
          "direction" -> writesOrientation(orientation)
        ))
  ...
  private def executeInstructions(lawn: Lawn, instructions: List[(Position, List[Instruction])]): List[(Position, Orientation, List[Instruction])] = {
    instructions.map { case (position, instructions) =>
      val mower = Mower(position, lawn)
      ...
      (finalMower.position, finalMower.position.orientation,instructions)
    }
  }
}
